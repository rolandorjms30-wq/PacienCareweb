...
navLinks.forEach(function(link) {
  link.addEventListener('click', function(e) {
    ...
    const nombreSeccion = link.dataset.seccion;
    
    // 1. Buscamos el elemento
    const seccion = document.getElementById(nombreSeccion);

    // 2. VERIFICACIÓN: Si no existe, avisamos en consola y salimos
    if (!seccion) {
      console.error("Error: No se encontró la sección con ID: " + nombreSeccion);
      return; 
    }

    // Continuamos solo si la sección existe
    navLinks.forEach(function(l) { l.classList.remove('activo'); });
    link.classList.add('activo');

    secciones.forEach(function(s) { s.classList.remove('activa'); });
    
    // Ahora es seguro usar classList
    seccion.classList.add('activa');

    tituloTop.textContent = link.querySelector('span:last-child').textContent;

    // Llamadas a funciones
    if (nombreSeccion === 'dashboard') cargarDashboard();
    cargarGraficos();
    if (nombreSeccion === 'pacientes') cargarPacientes();
    if (nombreSeccion === 'medicos') cargarMedicos();
    if (nombreSeccion === 'citasmedicas') cargarCitasMedicas();
    if (nombreSeccion === 'medicamentos') cargarMedicamentos();
    if (nombreSeccion === 'resultadosExamenes') cargarResultadosExamenes();
  });
});
// ======================================================
// ESTADO API
...
async function cargarDashboard() {
  try {
    ...
    const [pacientes, medicos, citas, examenes, medicamentos] = await Promise.all([
        fetch(API_LOCAL + '/catalogos/Pacientes/').then(r => r.json()),
        fetch(API_LOCAL + '/catalogos/Medicos/').then(r => r.json()),
        fetch(API_LOCAL + '/catalogos/CitasMedicas/').then(r => r.json()),      
        fetch(API_LOCAL + '/catalogos/ResultadosExamenes/').then(r => r.json()), 
        fetch(API_LOCAL + '/catalogos/Medicamentos/').then(r => r.json())  
    ]);
    ...
    document.getElementById('totalCitasMedicas').textContent = citas.length;
    document.getElementById('totalResultadosExamenes').textContent = examenes.length;
